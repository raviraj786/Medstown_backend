...
router.post("/orderMedicine", async (req, res) => {
  ...
  for (let i = 0; i < pharmacies.length; i++) {
    const url =
      ...
      lat +
      "&destinations=" +
      pharmacies[i].location.coordinates[0] +s
    "," +
      pharmacies[i].location.coordinates[1] +
      "&key=" +
      process.env.GOOGLE_MAPS_API_KEY +
...
router.post("/orderMedicines", async (req, res) => {
  try {
    const { customerId, orderDetails, quantity, lat, lng, orderId, totalPrice } = req.body;
    // Input Validation
    if (!customerId || !orderDetails   || !lat || !lng || !orderId || !totalPrice) {
    ...
    for (let i = 0; i < pharmacies.length; i++) {
      ...
      try {
        ...
        if (response.data && response.data.rows && response.data.rows.length > 0 &&
          ...
          response.data.rows[0].elements[0].distance && response.data.rows[0].elements[0].distance.value) {
    ...
    const orderPromises = orderData.map((data, i) => {
      return new Promise(async (resolve, reject) => {
        try {
          const existingOrder = await airorder.findOne({ orderId: data.orderId });
          if (existingOrder && existingOrder.orderId === data.orderId && existingOrder.orderNature) {
    ...
    res.status(500).json({ message: "An error occurred while processing the orders" });
  }
});
...
// SEND TO ORDER DELIVERY BOYS

router.post("/sendOrderToDeliveryBoyS", async (req, res) => {
  const {
    orderDetails,
    userLat,
    userLng,
  ...
  const options = {
    location: {
      $geoWithin: {
        $centerSphere: [[ pharmacyLat , pharmacyLng], 30 / 3963.2], // 30 miles = 48.2803 km
      },
    },
  };
  const deliveryBoy = await deliveryboydb.find(options);
...
router.get("/getOrder/pharmacy/:pharmacyId" , async(req,res) => {
  try {
...
const { customerId , orderId , orderDetails   } = req.body ;
const OTP =  Math.floor(Math.random(0,10))
console.log(OTP)
