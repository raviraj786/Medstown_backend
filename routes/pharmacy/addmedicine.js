...
router.get("/getmedicinebyid/:id/:db", async (req, res) => {
  try {
    if (req.params.db == "prescription") {
      const medicine = await Prescriptiondb.findOne({
...
router.get("/getmedicinebydisease/:disease", async (req, res) => {
  try {
    const medicine = await nonprescriptiondb.find({
...
router.get("/getmedicinebydiseasepage/:disease/:page", async (req, res) => {
  try {
    const [nonPrescriptionMeds, prescriptionMeds] = await Promise.all([
      nonprescriptiondb
        .find({ disease: req.params.disease })
        .skip((req.params.page - 1) * 20)
        .limit(20),
      Prescriptiondb.find({ disease: req.params.disease })
        .skip((req.params.page - 1) * 20)
        .limit(20),
    ]);
...
router.get("/getmedicinebysearch/:search", async (req, res) => {
  try {
    const medicine = await Medicinedb.find({
      medicineName: { $regex: req.params.search, $options: "i" },
    });
    res.send(medicine);
  } catch (err) {
    res
...
router.get("/searchmedicine/:search", async (req, res) => {
  try {
    const medicine = await searchmeddb
      .find({ medicineName: { $regex: req.params.search, $options: "i" } })
      .limit(10);
    res.send(medicine);
  } catch (err) {
    res
...
router.get("/getalldisease", async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send({ msg: "Something went wrong while fetching disease" });
  }
});
...
router.post(
  "/uploadmedicineimage/:id",
  upload.fields([{ name: "medicineImage", maxCount: 4 }]),
  async (req, res) => {
    ...
    try {
      const medicine = await Medicinedb.findOne({ medicineId: medicineId });
