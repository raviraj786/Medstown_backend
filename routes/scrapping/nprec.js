    ...
    for (let i = 0; i < data.anchors.length; i++) {
      ...
      const data3 = await page.evaluate(() => {
        ...
        return {
          ...
          disease: document.querySelector(
            "#maincontent > div.content-section > div.product-top > div.product-right-block > div.product-detail > a:nth-child(3) > span"
          )?.innerText,
          rxRequired:
          ...
          // medicinePrice: document
          //   .querySelector(".product-top .essentials .price-box .final-price")
          //   ?.innerText.replace("₹", ""),
      ...
      for (let j = 0; j < data3.medicineImage.length; j++) {
        ...
        let params = {
          ...
          ContentType:
            "image/jpeg, image/png, image/jpg , image/gif , image/svg+xml , image/webp , image/avif, image/svg",
        };
